...
【第三次迭代于然负责】
(1) 角色体系重构：新增运营总监、副总经理、财务总监、技术总监；角色重命名（Updated desc→总经理，测试角色→项目组长，数据查看员→数据分析员）；角色排序优化
(2) 组织架构同步功能：支持增加/删除/转移部门下的用户，用户部门变更自动同步到组织架构
(3) 用户管理编辑页面增加多重身份支持：一个员工可拥有多个部门下的多个职位和角色
(4) 数据分析员角色增加审计日志查看权限
